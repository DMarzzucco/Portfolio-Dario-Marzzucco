import NavImage from "../../public/img/Nav.webp"
import logo2Image from "../../public/img/logo2.webp"
import profileImage from "../../public/img/profile.webp";
import HomeImage from "../../public/img/Home.webp";
import Logo1Image from "../../public/img/logo1.webp";

import { faAddressCard, faCode, faPhone, faBars, faXmark, faEnvelope } from "@fortawesome/free-solid-svg-icons";

import { faDiagramProject } from "@fortawesome/free-solid-svg-icons/faDiagramProject";

import { faGithub, faGoogle, faLinkedin, faTelegram } from "@fortawesome/free-brands-svg-icons";



export { NavImage, logo2Image, profileImage, HomeImage, Logo1Image, faAddressCard, faCode, faPhone, faBars, faXmark, faDiagramProject, faGithub, faGoogle, faLinkedin, faTelegram, faEnvelope }